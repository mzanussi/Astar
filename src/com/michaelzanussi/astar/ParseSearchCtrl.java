...
public class ParseSearchCtrl {
	...
	public static int parse( Lexer lexer ) throws ParsingException {
		...
		if( !option.equals( "OpenListBound" ) &&
			!option.equals( "TotalNodesBound" ) &&
			!option.equals( "TimeBound" ) ) {
			
			throw new ParsingException( "ParseSearchCtrl.parse error: Invalid SEARCH-CTRL option: " +
					token.getToken() );
		...
		if( !token.getToken().equals( "=" ) ) {
			...
			throw new ParsingException( "ParseSearchCtrl.parse error: Expected '=' but received '" + 
					token.getToken() + "'." );
